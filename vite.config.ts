...
export default defineConfig(async ({ mode }): Promise<UserConfig> => ({
  ...
  clearScreen: false,
  test: {
    // Parallel-sprint worktrees live under .claude/worktrees/. Vitest's
    // defaults would scan those sibling checkouts and run other branches'
    // (possibly unmerged) tests against this checkout's source via the `@`
    // alias, producing spurious failures. Only run this project's own tests.
